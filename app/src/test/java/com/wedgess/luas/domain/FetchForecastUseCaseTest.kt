...
class FetchForecastUseCaseTest {
    ...
    fun setup() {
        ...
        fetchForecastUseCase = FetchForecastUseCase(luasRepository)
    }

    @Test
    fun `invoke should emit success states with increasing progress in AUTOMATIC mode`() = runTest {
        val results = mutableListOf<RefreshState<ForcastEntity>>()
        val job = launch { fetchForecastUseCase(stopAbv).take(5).toList(results) }
        ...
        val progressValues =
            results.filterIsInstance<RefreshState.Success<ForcastEntity>>().map { it.progress }
    ...
    fun `invoke should emit error state when repository fails`() = runTest {
        ...
        val result = fetchForecastUseCase(stopAbv).first()
    ...
    fun `refresh should change refresh mode and trigger refresh flow`() = runTest {
        fetchForecastUseCase.refresh(RefreshMode.MANUAL)
        val result = fetchForecastUseCase(stopAbv).first()
