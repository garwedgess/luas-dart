...
class ForecastTabViewModelTest {
    ...
    fun setUp() {
        ...
        viewModel = ForecastTabViewModel(
            luasLine = luasLine,
            fetchStopsUseCase = fetchStopsUseCase,
            fetchForecastUseCase = fetchForecastUseCase,
            updateSelectedStationUseCase = updateSelectedStationUseCase,
    ...
    fun `should update selected stop when OnStopSelected event is received`() = runTest {
        ...
        // When
        viewModel.onEvent(ForecastTabContract.Event.OnStopSelected(selectedStop))

        // Then
        coVerify { updateSelectedStationUseCase(selectedStop.abbreviation, selectedStop.line) }
    }

    @Test
    fun `should show travel updates dialog when OnShowTravelUpdatesDialog event is received`() = runTest {
        // When
        viewModel.onEvent(ForecastTabContract.Event.OnShowTravelUpdatesDialog)
    ...
    fun `should trigger manual refresh when OnRefresh event is received`() = runTest {
        ...
        // When
        viewModel.onEvent(ForecastTabContract.Event.OnRefresh)
    ...
    fun `timer should stop when all minutes are elapsed`() = runTest {
        ...
        // When
        viewModel.onEvent(ForecastTabContract.Event.OnStartNotification(minutes = 1))
    ...
    fun `should handle non-existent stop when OnStopSelected is called`() = runTest {
        ...
        // When
        viewModel.onEvent(ForecastTabContract.Event.OnStopSelected(nonExistentStop))

        // Then should still attempt to update the selected station
        coVerify { updateSelectedStationUseCase(nonExistentStop.abbreviation, nonExistentStop.line) }
    }
    ...
    fun `timer should be cancelled when OnStopNotification event is received`() = runTest {
        ...
        viewModel.onEvent(ForecastTabContract.Event.OnStopNotification)
    ...
    fun `should handle update of selected station failure gracefully`() = runTest {
        // Given
        coEvery { updateSelectedStationUseCase(any(), any()) } returns Result.failure(Exception("Database error"))

        // When - should not crash
        viewModel.onEvent(ForecastTabContract.Event.OnStopSelected(mockStops[1]))
