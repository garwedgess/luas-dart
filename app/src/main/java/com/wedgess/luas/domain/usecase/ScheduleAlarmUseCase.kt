...
class ScheduleAlarmUseCase @Inject constructor(
    ...
    suspend operator fun invoke(secondsFromNow: Long, luasNotificationEntity: LuasNotificationEntity): Result<Long> {
        require(secondsFromNow > 0) { "Minutes must be greater than 0" }
        return alarmRepository.scheduleAlarm(secondsFromNow, luasNotificationEntity)
    }
}
